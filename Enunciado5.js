const filmes = [
  ...
  function filmes2000() {
    const filmesFiltrados = filmes.filter(filme =>  filme.lancamento < 2000);
    const arr = [];
     filmesFiltrados.forEach(filtrado => {
      arr.push({
        titulo: filtrado.nome,
        generos: filtrado.generos,
      })
   });
   return arr;
  }

const resultado = filmes2000();
